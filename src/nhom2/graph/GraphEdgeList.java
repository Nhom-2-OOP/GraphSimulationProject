...
public class GraphEdgeList<V, E> implements Graph<V,E>{
	...
	public Map<Vertex<V>, Map<Vertex<V>,Edge<E,V>>> adjListReverse;
	...
	public GraphEdgeList(boolean isDirectedGraph) {
    	...
    	this.adjListReverse = new HashMap<>();
	...
	@Override
	public Collection<Vertex<V>> adjVertexList(Vertex<V> v) {
		return adjList.get(v).keySet();
	}

	@Override
	public Collection<Vertex<V>> adjVertexListReverse(Vertex<V> v) {
		return adjListReverse.get(v).keySet();
	}
	
	@Override
	public Collection<Edge<E, V>> incidentEdgesReverse(Vertex<V> v) {
		return adjListReverse.get(v).values();
	}
	@Override
	// DPT: O(n)
	...
	public Vertex<V> insertVertex(V vElement) {
		...
		adjListReverse.put(NewVertex, newMapOne);
	...
	public Edge<E, V> insertEdge(V vElement1, V vElement2, E edgeElement) {
        ...
        if(this.isDirected) {
        	adjListReverse.get(inVertex).put(outVertex, newEdge);
        }
        if (!this.isDirected) {
	...
	@Override
	public void deleteEdge(Edge<E, V> edge) {
	...
	public void deleteVertex(Vertex<V> vertex) {
		...
		this.adjListReverse.remove(vertex);
		this.NumOfEdge.remove(vertex);
		
		for(Edge<E, V> edge : this.incidentEdges(vertex)) {
			deleteEdge(edge);
		}
		
		for(Edge<E, V> edge : this.incidentEdgesReverse(vertex)) {
			deleteEdge(edge);
		}
	}
